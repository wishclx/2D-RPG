...
public class UI_SkillToolTip : UI_ToolTip
{
    ...
    public void ShowToolTip(bool show, RectTransform targetRect, SkillDataSO skillData, UI_TreeNode node)
    {
        ...
        skillCoolDown.text = "技能CD:" + skillData.upgradeData.cooldown + "s";
    ...
    {
        StopLockSkillEffect();// 确保之前的效果被停止，避免叠加。
    ...
    private IEnumerator TextBlinkEffectCo(TextMeshProUGUI text, float blinkInterval, int blinkCount)
    {
        for (int i = 0; i < blinkCount; i++)
        {
            ...
            yield return new WaitForSecondsRealtime(blinkInterval); // 使用真实时间，暂停时也能继续

            text.text = GetColoredText(importantInfoHex, lockedSkillText);// 切回重要提示颜色
            yield return new WaitForSecondsRealtime(blinkInterval); // 使用真实时间，暂停时也能继续
        }

        // 确保最终状态是重要提示颜色，避免中途停止后颜色残留
